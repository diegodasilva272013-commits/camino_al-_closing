...
export async function Topbar({ isAdmin = false, role = 'student', newSignupsToday = 0 }: { isAdmin?: boolean; role?: string; newSignupsToday?: number }) {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center gap-2 border-b border-[rgba(212,175,55,0.12)] bg-[#0a0a0a]/90 px-3 backdrop-blur sm:gap-3 sm:px-4 lg:px-8">
      <MobileNav isAdmin={isAdmin} role={role} newSignupsToday={newSignupsToday} />
