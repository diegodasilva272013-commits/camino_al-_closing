...
// Mismo prompt de evaluación que el módulo individual
...
Devolvé JSON con los mismos campos que siempre.`;
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data: ca } = await admin
    ...
    .select('id, team_id, analysis, status')
    .eq('id', params.id)
    .single();
  ...
  // Verificar que el setter pertenece al equipo
  const { data: team } = await admin
    ...
    .single();
  if (!team) return NextResponse.json({ error: 'No pertenecés a este equipo' }, { status: 403 });

  // No puede reflexionar dos veces
  ...
  for (const f of required) {
    if (!answers[f]?.trim() || answers[f].trim().length < 20) {
      return NextResponse.json({ error: `Respuesta muy corta en "${f}". Mínimo 20 caracteres.` }, { status: 400 });
    }
  }
  ...
  try {
    ...
    await admin.from('team_conversation_reflections')
      .insert({ analysis_id: params.id, user_id: user.id, answers, evaluation, xp_earned, status });

    // Sumar XP al perfil individual del setter — mismas reglas que el módulo individual
    if (xp_earned > 0) {
