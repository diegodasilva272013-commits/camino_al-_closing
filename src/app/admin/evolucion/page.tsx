...
export default async function EvolucionDashboard() {
  ...
  const [{ data: personas }, { data: patrones }] = await Promise.all([
    ...
    admin
      .from('patrones')
      .select('*, persona:personas(nombre), capacidad:capacidades(nombre)')
      .gt('frecuencia', 1)
  ...
  return (
    <div className="min-h-screen bg-[#080808] px-4 py-6 lg:px-8">
      <div className="flex items-start justify-between gap-4">
        <PageHeader
          eyebrow="Admin · Sistema de Evolución"
          title="Dashboard Evolución CAC"
      ...
      {personaList.length === 0 ? (
        ...
        <div className="mt-6 space-y-4 max-w-5xl">
          {personaList.map((persona) => {
            ...
            const positivos  = pats.filter((p: any) => (p as any).tipo === 'positivo' || true); // all patterns
                    ...
                    {topPatrons.map((pat: any) => (
                      ...
                      >
                        <div className="flex items-center gap-2 min-w-0">
                          <span className={`shrink-0 h-1.5 w-1.5 rounded-full ${pat.tipo === 'negativo' ? 'bg-red-500' : 'bg-emerald-500'}`} />
