-- =====================================================================
-- 0039 · Ranking real de setters
-- Agrega puntos por trabajo concreto en tablas ya existentes.
-- No crea tablas nuevas. SECURITY DEFINER para leer todas las filas.
-- Pegar en Supabase → SQL Editor → Run.
-- =====================================================================

-- Pesos:
--   conversación analizada   → +10
--   sesión trainer completada → +8
--   formulario analizado     → +15
--   post en comunidad        → +5
--   comentario               → +2
--   like recibido en post    → +1
...
as $$
  with
  -- ventana de tiempo (0 = histórico completo)
  since as (
    select case
  ...
  -- solo roles que trabajan como setters
  setters as (
    select id, full_name, avatar_url
    from public.profiles
    where role in ('setter', 'admin', 'mentor')
  ),
  -- conversaciones analizadas exitosamente
  conv as (
  ...
  -- sesiones de entrenamiento completadas (ended_at no nulo)
  sess as (
    select user_id, count(*)::int as cnt
  ...
  -- formularios de refuerzo analizados
  forms as (
    select user_id, count(*)::int as cnt
  ...
  -- actividad en comunidad: posts + comentarios + likes recibidos
  comm_posts as (
    select user_id, count(*)::int as cnt
